...
class OpenlineageSource(PipelineServiceSource):
    ...
    def get_create_topic_request(self, topic: Dict) -> Optional[Either]:
        topic_details = OpenlineageSource._get_topic_details(topic)
        topic_fqn = None
            ...
            for io_entity in io_entities:
                if OpenlineageSource._is_kafka_topic(io_entity):
                    ...
                    table_fqn = self._get_table_fqn(
                        OpenlineageSource._get_table_details(io_entity)
                    )
